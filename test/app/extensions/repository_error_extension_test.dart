...
void main() {
  group('RepositoryErrorExtension - translateError', () {
    ...
    testWidgets('authExpired returns localized string', (tester) async {
      ...
      final result = const RepositoryError.authExpired().translateError(
        context,
      );
      expect(result, AppLocalizations.of(context).authenticationExpired);
    });

    testWidgets('badRequestListErrors returns joined error list', (
      tester,
    ) async {
      await _pumpWithContext(tester, const Locale('en'));
      final result = const RepositoryError.badRequestListErrors([
        'Error 1',
        'Error 2',
      ]).translateError(context);
      expect(result, 'Error 1\nError 2');
    });

    testWidgets('userNotFound returns localized string', (tester) async {
      await _pumpWithContext(tester, const Locale('en'));
      final result = const RepositoryError.userNotFound().translateError(
        context,
      );
    ...
    testWidgets('notFoundResource returns localized string', (tester) async {
      await _pumpWithContext(tester, const Locale('en'));
      final result = const RepositoryError.notFoundResource().translateError(
        context,
      );
    ...
    testWidgets('listErrors returns joined string', (tester) async {
      await _pumpWithContext(tester, const Locale('en'));
      final result = const RepositoryError.listErrors([
        'A',
        'B',
      ]).translateError(context);
      expect(result, 'A\nB');
    ...
    testWidgets('badRequest returns localized string', (tester) async {
      await _pumpWithContext(tester, const Locale('en'));
      final result = const RepositoryError.badRequest().translateError(context);
      expect(result, AppLocalizations.of(context).badRequest);
    });

    testWidgets('infoNotMatching returns localized string', (tester) async {
      await _pumpWithContext(tester, const Locale('en'));
      final result = const RepositoryError.infoNotMatching().translateError(
        context,
      );
    ...
    testWidgets('noAccess returns localized string', (tester) async {
      await _pumpWithContext(tester, const Locale('en'));
      final result = const RepositoryError.noAccess().translateError(context);
      expect(result, AppLocalizations.of(context).noAccess);
    });

    testWidgets('noInternetConnection returns localized string', (
      tester,
    ) async {
      await _pumpWithContext(tester, const Locale('en'));
      final result = const RepositoryError.noInternetConnection()
          .translateError(context);
    ...
    testWidgets('serverError returns localized string', (tester) async {
      await _pumpWithContext(tester, const Locale('en'));
      final result = const RepositoryError.serverError().translateError(
        context,
      );
    ...
    testWidgets('securityError returns localized string', (tester) async {
      await _pumpWithContext(tester, const Locale('en'));
      final result = const RepositoryError.securityError().translateError(
        context,
      );
    ...
    testWidgets('wrongPassword returns localized string', (tester) async {
      await _pumpWithContext(tester, const Locale('en'));
      final result = const RepositoryError.wrongPassword().translateError(
        context,
      );
    ...
    test('snackBarType returns correct type', () {
      const userNotFoundError = RepositoryError.userNotFound();
      const wrongPasswordError = RepositoryError.wrongPassword();
      const defaultError = RepositoryError.serverError();

      expect(userNotFoundError.snackBarType, SnackBarTypeEnum.warning);
      expect(wrongPasswordError.snackBarType, SnackBarTypeEnum.warning);
      expect(defaultError.snackBarType, SnackBarTypeEnum.error);
    });
  });
}
