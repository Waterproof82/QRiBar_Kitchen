...
void main() {
  group('LoginScreen Widget Tests', () {
    testWidgets('LoginScreen renders all expected child widgets', (
      WidgetTester tester,
    ) async {
      ...
      final Finder outerColumnFinder = find.descendant(
        ...
        matching: find.byWidgetPredicate(
          (widget) =>
              widget is Column &&
              widget.key == const Key('loginScreenOuterColumn'),
        ), // Add a Key to this Column in LoginScreen
      );
      expect(outerColumnFinder, findsOneWidget);

      // Verify the Row with LanguageDropdown inside the OUTER Column
      expect(
        find.descendant(of: outerColumnFinder, matching: find.byType(Row)),
        findsOneWidget,
        );
      ...
      expect(
        find.descendant(
          of: outerColumnFinder,
          matching: find.byType(LoginContainer),
        ),
        findsOneWidget,
      );
      ...
      final Finder innerColumnFinder = find.descendant(
        ...
        matching: find.byWidgetPredicate(
          (widget) =>
              widget is Column &&
              widget.key == const Key('loginContainerInnerColumn'),
        ),
      );
      ...
      expect(
        find.descendant(of: innerColumnFinder, matching: find.text('Login')),
        findsOneWidget,
      );
